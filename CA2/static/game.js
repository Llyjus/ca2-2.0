...
function attackActivate(event) {
    ...
    console.log(mapPositionX)
    console.log(mapPositionY)
    //skill
...
function drawMain() {
    ...
    context.beginPath();
    context.arc(player.x - viewport.x, player.y - viewport.y, player.xSize / 2, 0, Math.PI * 2);
    context.closePath();
    context.fill();
...
function gameOver() {
    ...
    let record;
    if (stairLevel >= 5) {
        record = "Your game has been recorded. it may be displayed on the leaderboard. Cheers!"
...
function load_assets(assets, callback) {
    ...
    for (let asset of assets) {
        ...
        if (element instanceof HTMLImageElement) {
            console.log('img');
            element.addEventListener('loading', loaded, false);
        }
